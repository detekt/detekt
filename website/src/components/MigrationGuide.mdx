...
The complexity report was moved from core to a first party plugin. If you want to get the complexity metrics of your project add
`detektPlugin("dev.detekt:detekt-report-complexity:2.0.0")` to your `build.gradle` dependencies.

The statistics report was moved from core to a first party plugin. If you want to get the statistics of your project add
`detektPlugin("dev.detekt:detekt-report-statistics:2.0.0")` to your `build.gradle` dependencies.
...
Also, the YAML configuration parser is now stricter with the types. Before it parsed `active: "true"` correctly but
that's not the case anymore. You need to use the correct YAML types: `active: true`.
