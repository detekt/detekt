...
detekt requires **Gradle 6.8.3** or higher. We, however, recommend using the version of Gradle that is [listed in this table](../introduction/compatibility.mdx).
...
[type resolution](type-resolution.mdx) enabled. This happens for both, pure JVM projects and Android projects that have
...
More information on type resolution are available on the [type resolution](type-resolution.mdx) page.
...
Instead of disabling detekt for the check task, you may want to use the [baseline feature](../introduction/baseline.mdx).
