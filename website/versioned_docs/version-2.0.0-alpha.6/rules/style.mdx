...
* ``ignoreSealedClasses`` (default: ``false``)

  if sealed classes should be ignored. `sealed class` dispatches virtually while `sealed interface` dispatches through an interface table, so projects with heavy sealed hierarchies may prefer to keep classes and opt out of this check.
...
* ``ignoredFullyQualifiedNames`` (default: ``[]``)

  fully qualified names that are ignored, for example `['java.lang.String', 'kotlin.**.*', 'java.util.*List', 'java.net.UR?']`
