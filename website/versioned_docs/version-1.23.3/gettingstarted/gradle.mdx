...
You can pin only the detekt version to a supported one, this may cause incorrect analysis results as detekt is running
on a different kotlin version than the project is compiled with:
