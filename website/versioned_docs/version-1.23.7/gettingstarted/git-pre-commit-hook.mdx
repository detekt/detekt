...
Please note, however, that a handful of checks requiring [type resolution](type-resolution.mdx) will not work correctly with this approach. If you do adopt a partial hook, it is recommended that you still implement a full `detekt` check as part of your CI pipeline.
