...
* ``allowedComplexity`` (default: ``15``)

  Maximum Cognitive Complexity allowed for a method.
...
* ``allowedConditions`` (default: ``3``)

  Maximum allowed number of conditions.
...
* ``allowedDefinitions`` (default: ``10``)

  The amount of allowed definitions in an interface.
...
* ``allowedComplexity`` (default: ``14``)

  The maximum allowed McCabe's Cyclomatic Complexity (MCC) for a method.
...
* ``ignoreLocalFunctions`` (default: ``false``)

  Whether to ignore local functions and count them as one
...
* ``allowedLines`` (default: ``600``)

  The maximum number of lines allowed per class.
...
* ``allowedLines`` (default: ``60``)

  number of lines in a method that are allowed at maximum
...
* ``allowedFunctionParameters`` (default: ``5``)

  number of function parameters required to trigger the rule

* ``allowedConstructorParameters`` (default: ``6``)
...
* ``allowedOverloads`` (default: ``6``)

  The allowed number of overloads for a method.
...
* ``allowedArguments`` (default: ``3``)

  The allowed number of arguments for a function.

* ``ignoreMethods`` (default: ``[]``)

  List of fully qualified method signatures where this rule is ignored. Methods can be defined without full signatures (i.e. `java.time.LocalDate.now`), which will report calls of all methods with this name or with the full signature (i.e. `java.time.LocalDate(java.time.Clock)`), which reports only callswith the specific signature given. If you want to add an extension function like `fun String.hello(a: Int)` you should add the receiver parameter as the first parameter like this: `hello(kotlin.String, kotlin.Int)`. To add constructor calls you need to define them with `<init>`, for example `java.util.Date.<init>`. To add calls involving type parameters, omit them, for example `fun hello(args: Array<Any>)` is referred to as simply `hello(kotlin.Array)`. To add calls involving varargs, for example `fun hello(vararg args: String)`, you need to define it like `hello(vararg String)`. To add methods from the companion object reference the companion class, for example with `TestClass.Companion.hello()`, even if it is marked `@JvmStatic`.
...
* ``allowedDepth`` (default: ``4``)

  The maximum allowed nested block depth for a function
...
* ``allowedDepth`` (default: ``1``)

  The maximum allowed depth for nested scope functions.
...
* ``allowedDuplications`` (default: ``2``)

  The maximum allowed amount of duplications.

* ``ignoreAnnotation`` (default: ``true``)

  if values in Annotations should be ignored

* ``allowedWithLengthLessThan`` (default: ``5``)

  The maximum string length below which duplications are allowed
...
* ``allowedFunctionsPerFile`` (default: ``11``)

  The maximum allowed functions per file

* ``allowedFunctionsPerClass`` (default: ``11``)

  The maximum allowed functions per class

* ``allowedFunctionsPerInterface`` (default: ``11``)

  The maximum allowed functions per interface

* ``allowedFunctionsPerObject`` (default: ``11``)

  The maximum allowed function per object

* ``allowedFunctionsPerEnum`` (default: ``11``)

  The maximum allowed functions in enums
