...
Please note that rules inside the [`formatting`](../rules/formatting.mdx) ruleset can only be suppressed at **the file level**.

Rules inside this ruleset are wrappers around KtLint rules, and we don't have the same reporting capabilities that we offer for first party rules. For example, you can suppress the [MaximumLineLength](../rules/formatting.mdx#maximumlinelength) rule only in your entire file with:
...
Several rules in the [`formatting`](../rules/formatting.mdx) ruleset also have a "first party" counterpart. For instance you can use the [`MaxLineLength`](../rules/style.mdx#maxlinelength) rule instead from the [`style`](../rules/style.mdx) ruleset.
