...
detekt requires **Gradle 6.8.3** or higher. We, however, recommend using the version of Gradle that is [listed in this table](../introduction/compatibility.md).
...
`detekt` closure. 
  ...
  - Reports are automatically generated in checkstyle(xml),
...
- `detektBaseline` - Similar to `detekt`, but creates a finding baseline. Further detekt runs will only feature new findings not in this list.
- `detektProfile` - Runs detekt with profiling enabled to measure rule execution times. Displays the top slowest
  rules in console output and generates an aggregated profiling summary. In multi-project builds, running this task 
  on the root project will also run profiling on all subprojects and aggregate the results into 
  `build/reports/detekt/detektProfile-aggregate.txt`. Note: Parallel execution is automatically disabled when profiling 
...
- `detektProfileMain` - Similar to `detektProfile`, but runs profiling only on the `main` source set with type 
  resolution enabled (Android: all production source sets). In multi-project builds, running this task on the root 
  project will aggregate profiling results from all subprojects into `build/reports/detekt/detektProfileMain-aggregate.txt`.
- `detektProfileTest` - Similar to `detektProfile`, but runs profiling only on the `test` source set with type 
  resolution enabled (Android: all JVM and Android Test source sets). In multi-project builds, running this task on 
  the root project will aggregate profiling results from all subprojects into `build/reports/detekt/detektProfileTest-aggregate.txt`.
...
- Android-only: `detektProfile<Variant>` - Similar to `detektProfile`, but runs profiling with type resolution 
  enabled only for the specific (test) build variant. In multi-project builds, running this task on the root project 
  will aggregate profiling results from all subprojects.
...
buildscript {
    ...
    dependencies {
        classpath "dev.detekt:detekt-gradle-plugin:[detekt_version]"
    }
}
...
tasks.named("detekt").configure {
    reports {
        ...
        markdown.outputLocation.set(file("build/reports/detekt.md"))
        custom {
...
`jdkHome` is set automatically when applying a toolchain using either
...
Instead of disabling detekt for the check task, you may want to use the [baseline feature](../introduction/baseline.md).
