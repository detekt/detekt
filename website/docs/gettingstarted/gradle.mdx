...
detekt {
    ...
    // If set to `true` the build does not fail when there are issues. Defaults to `false`.
    ignoreFailures = false

    // The build to fails when there is at least one issue with this severity (or above).
    // Allowed values are "error" (default), "warning", "info" and "never". If set to "never",
    // the build will not fail regardless of the number of issues and their severities.
