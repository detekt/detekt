...
Instead of disabling detekt for the check task, you may want to increase the build failure threshold in the [configuration file](/docs/introduction/configurations).
...
```
detekt was compiled with Kotlin x.x.x but is currently running with y.y.y
```
...
in combination with the [Spring boot plugin](https://docs.spring.io/spring-boot/docs/3.2.0/gradle-plugin/reference/htmlsingle/#introduction):

Below are two workarounds, be aware that both come with their own drawbacks.

You can pin the whole projects kotlin version to the one supported by detekt:
...
You can pin the detekt version to a supported one by adding the following to your build gradle file, this may cause
incorrect analysis results as detekt is running on a different kotlin version than the project is compiled with:
