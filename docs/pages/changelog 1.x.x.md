...
that are using JetPack compose with features such as [`ignoreAnnotated` and `ignoreFunction`](https://detekt.dev/suppressors.html).
...
We added documentation on how to configure [type resolution](https://detekt.dev/type-resolution.html).  
...
for example https://detekt.dev/style.html#forbiddenmethodcall).
...
- [detekt runs can be completely silent on absence of findings](https://detekt.dev/howto-silent-reports.html)
