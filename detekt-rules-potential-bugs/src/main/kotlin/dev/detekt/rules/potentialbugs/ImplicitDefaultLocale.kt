...
class ImplicitDefaultLocale(config: Config) :
    ...
    private fun getFormatString(expression: KtQualifiedExpression): String? {
        ...
        if (receiver is KtStringTemplateExpression) {
            // Extension style: "format".format(args)
            // Return the format string only if it doesn't have interpolation
            return if (!receiver.hasInterpolation()) {
                receiver.entries.joinToString("") { it.text }
            } else {
                null
            }
        }
        // Static style: String.format("format", args)
    ...
    private fun hasOnlyLocaleIndependentSpecifiers(formatString: String): Boolean {
        ...
        return conversions.isEmpty() || conversions.all { it in localeIndependentConversions }
    }
    ...
    companion object {
        ...
        private val specifierPattern = Regex("""%(\d+\$)?[-#+ 0,(]*\d*(\.\d+)?([tT][a-zA-Z]|[a-zA-Z%])""")
