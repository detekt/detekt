...
class IgnoredReturnValueSpec(private val env: KotlinCoreEnvironment) {
    ...
    inner class `custom annotation config` {
        private val subject =
            IgnoredReturnValue(TestConfig(mapOf("returnValueAnnotations" to listOf("*.CustomReturn"))))
    ...
    inner class `restrict to annotated methods config` {
        private val subject = IgnoredReturnValue(TestConfig(mapOf("restrictToAnnotatedMethods" to false)))
