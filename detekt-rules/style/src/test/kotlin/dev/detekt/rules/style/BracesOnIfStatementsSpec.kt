...
import dev.detekt.rules.style.BracesOnIfStatements.BracePolicy
import dev.detekt.rules.style.BracesOnIfStatementsSpec.Companion.NOT_RELEVANT
import dev.detekt.rules.style.BracesOnIfStatementsSpec.Companion.options
import dev.detekt.rules.style.BracesOnIfStatementsSpec.Companion.test
...
class BracesOnIfStatementsSpec {
    ...
    inner class singleLine {
        ...
        inner class `=never` {
            ...
            @TestFactory fun `fully extra braces are flagged`() =
                listOf(
                    flag("if (true) { println() }", "if"(1)),
                    flag("if (true) { println() } else { println() }", "if"(1), "else"(1)),
                    flag("if (true) { println() } else if (true) { println() }", "if"(1), "if"(2)),
        ...
        inner class `=necessary` {
            ...
            @TestFactory
            fun `extra braces are flagged when not necessary (first)`() =
                listOf(
            ...
            @TestFactory
            fun `extra braces are flagged when not necessary (last)`() =
                listOf(
    ...
    inner class multiLine {
        ...
        inner class `=consistent` {
            ...
            @TestFactory
            fun `no braces in chained expression are accepted`() =
                listOf(
                    flag(
                        """
                            if (true) {
    ...
    inner class nested {
        ...
        @TestFactory
        fun `nested ifs are flagged for consistency`() =
            testCombinations(
        ...
        @TestFactory
        fun `nested ifs are flagged for always`() =
            testCombinations(
        ...
        @TestFactory
        fun `nested ifs inside condition are flagged for always`() =
            testCombinations(
                singleLine = BracePolicy.Always.config,
        ...
        @TestFactory
        fun `nested ifs inside then are flagged for always`() =
            testCombinations(
                singleLine = BracePolicy.Always.config,
                multiLine = NOT_RELEVANT,
                code = """
