        ...
        ruleSetConfig.subConfigKeys()
            ...
            .mapNotNull { (ruleId, ruleName) ->
                ruleSet.rules[ruleName]?.let { ruleProvider ->
                    ...
                    val url =
                        "https://detekt.dev/docs/rules/${ruleSet.id.value.lowercase()}#${rule.ruleName.value.lowercase()}"
                    RuleDescriptor(
    ...
    .filter { ruleDescriptor ->
        if (fullAnalysis) {
            ...
            if (requiresFullAnalysis) {
                log {
                    "The rule '${ruleDescriptor.ruleInstance.id}' requires type resolution but it was run without it."
                }
            }
