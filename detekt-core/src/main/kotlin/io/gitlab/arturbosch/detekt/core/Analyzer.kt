    ...
    private fun analyze(
        ...
        fun executeRules(rules: List<BaseRule>) {
            for (rule in rules) {
                ...
                for (finding in filterSuppressedFindings(file, rule, bindingContext)) {
...
private fun filterSuppressedFindings(file: KtFile, rule: BaseRule, bindingContext: BindingContext): List<Finding> {
    val suppressors = getSuppressors(file, rule, bindingContext)
