...
class BaselineResultMapping : ReportingExtension {
    ...
    private var initBaselineFile: Path? = null
    private var createBaseline: Boolean = false

    override val id: String = "BaselineResultMapping"

    override fun init(context: SetupContext) {
        initBaselineFile = context.getOrNull(DETEKT_BASELINE_PATH_KEY)
    ...
    override fun transformIssues(issues: List<Issue>): List<Issue> {
        val localBaselineFile = initBaselineFile
        require(!createBaseline || (createBaseline && localBaselineFile != null)) {
            "Invalid baseline options invariant."
        }

        return localBaselineFile?.let { issues.transformWithBaseline(it) } ?: issues
    }
