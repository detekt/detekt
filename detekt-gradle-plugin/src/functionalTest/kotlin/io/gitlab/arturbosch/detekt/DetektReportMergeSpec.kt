...
class DetektReportMergeSpec {
    ...
    fun `Checkstyle merge is configured correctly for multi module project`() {
        ...
        val buildFileContent = """
            ...
            val checkstyleReportMerge by tasks.registering(io.gitlab.arturbosch.detekt.report.ReportMergeTask::class) {
            ...
            subprojects {
                ...
                checkstyleReportMerge {
                    input.from(tasks.withType<io.gitlab.arturbosch.detekt.Detekt>().map { it.reports.checkstyle.outputLocation })
                }
            }
        """.trimIndent()
