...
class DetektTaskGroovyDslSpec {
    ...
    fun `detekt extension can be configured without errors`() {
        val config = """
            detekt {
                ...
                ignoreFailures = true
                failOnSeverity = dev.detekt.gradle.plugin.extensions.FailOnSeverity.Info
    ...
    fun `detekt task can be fully configured without errors`() {
        val config = """
            tasks.create("customDetektTask", dev.detekt.gradle.plugin.Detekt) {
    ...
    fun `detekt create baseline task can be configured without errors`() {
        val config = """
            tasks.create("customDetektCreateBaselineTask", dev.detekt.gradle.plugin.DetektCreateBaselineTask) {
                source = files("${"$"}projectDir")
                includes = ["**/*.kt", "**/*.kts"]
                excludes = ["build/"]
                ignoreFailures = false
    ...
    fun `detekt generate config task can be configured without errors`() {
        val config = """
            tasks.create("customDetektGenerateConfigTask", dev.detekt.gradle.plugin.DetektGenerateConfigTask) {
                detektClasspath.setFrom(files("config.yml"))
