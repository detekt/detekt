...
import dev.detekt.gradle.plugin.extensions.loadDetektVersion
import dev.detekt.gradle.plugin.test.DslTestBuilder.Companion.kotlin
...
class DetektTaskDslSpec {
    ...
    inner class FailureSeverity {
        ...
        fun `can be configured`() {
            val config = """
                detekt {
                    failOnSeverity = dev.detekt.gradle.plugin.extensions.FailOnSeverity.Never
                }
            """.trimIndent()
    ...
    inner class `and creating a custom task` {
        ...
        private val config = """
            task<dev.detekt.gradle.plugin.Detekt>("myDetekt") {
                ...
                failOnSeverity = dev.detekt.gradle.plugin.extensions.FailOnSeverity.Error
                autoCorrect = false
                reports {
                    xml {
                        required.set(true)
