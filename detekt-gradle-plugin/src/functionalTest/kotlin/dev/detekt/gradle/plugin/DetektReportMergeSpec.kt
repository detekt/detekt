...
import dev.detekt.gradle.plugin.test.DslGradleRunner
import dev.detekt.gradle.plugin.test.DslTestBuilder
import dev.detekt.gradle.plugin.test.ProjectLayout
import dev.detekt.gradle.plugin.test.reIndent
...
class DetektReportMergeSpec {
    ...
    fun `Sarif merge is configured correctly for multi module project`() {
        ...
        val buildFileContent = """
            ...
            val sarifReportMerge by tasks.registering(dev.detekt.gradle.plugin.report.ReportMergeTask::class) {
            ...
            subprojects {
                ...
                sarifReportMerge {
                  input.from(tasks.withType<dev.detekt.gradle.plugin.Detekt>().map { it.reports.sarif.outputLocation })
                }
            }
        """.trimIndent()
    ...
    fun `XML merge is configured correctly for multi module project`() {
        ...
        val buildFileContent = """
            ...
            val xmlReportMerge by tasks.registering(dev.detekt.gradle.plugin.report.ReportMergeTask::class) {
            ...
            subprojects {
                ...
                xmlReportMerge {
                    input.from(tasks.withType<dev.detekt.gradle.plugin.Detekt>().map { it.reports.xml.outputLocation })
                }
            }
        """.trimIndent()
