...
class HtmlOutputReport : BuiltInOutputReport, OutputReport() {
    ...
    private fun FlowContent.renderRule(rule: Rule.Id, group: RuleSet.Id, issues: List<Issue>) {
        details {
            id = rule.value
            open = true

            summary("rule-container") {
                span("rule") { text("$rule: %,d ".format(Locale.ROOT, issues.size)) }
                span("description") { text(issues.first().ruleInfo.description) }
            }
